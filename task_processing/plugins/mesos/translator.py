...
def make_mesos_container_info(task_config: MesosTaskConfig) -> addict.Dict:
    ...
    port_mappings = [
        addict.Dict(host_port=task_config.ports[0]["begin"], container_port=8888)
    ]
...
def make_mesos_command_info(task_config: MesosTaskConfig) -> addict.Dict:
    return addict.Dict(
        ...
        environment=make_task_environment_variables(task_config=task_config),
    )
...
MESOS_STATUS_MAP = {
    "TASK_STARTING": addict.Dict(platform_type="starting", terminal=False),
    "TASK_RUNNING": addict.Dict(platform_type="running", terminal=False),
    "TASK_FINISHED": addict.Dict(platform_type="finished", terminal=True, success=True),
    "TASK_FAILED": addict.Dict(platform_type="failed", terminal=True, success=False),
    "TASK_KILLED": addict.Dict(platform_type="killed", terminal=True, success=False),
    "TASK_LOST": addict.Dict(platform_type="lost", terminal=True, success=False),
    "TASK_STAGING": addict.Dict(platform_type="staging", terminal=False),
    "TASK_ERROR": addict.Dict(platform_type="error", terminal=True, success=False),
    "TASK_KILLING": addict.Dict(platform_type="killing", terminal=False),
    "TASK_DROPPED": addict.Dict(platform_type="dropped", terminal=True, success=False),
    "TASK_UNREACHABLE": addict.Dict(platform_type="unreachable", terminal=False),
    "TASK_GONE": addict.Dict(platform_type="gone", terminal=True, success=False),
    "TASK_GONE_BY_OPERATOR": addict.Dict(
        platform_type="gone_by_operator", terminal=True, success=False
    ),
    "TASK_UNKNOWN": addict.Dict(platform_type="unknown", terminal=False),
    "TASK_STUCK": addict.Dict(platform_type="unknown", terminal=False),
}


def mesos_update_to_event(
    mesos_status: addict.Dict, task_config: MesosTaskConfig
) -> Event:
    kwargs = dict(
        raw=mesos_status,
