...
from task_processing.plugins.mesos.constraints import valid_constraint_operator_name
...
def _valid_constraints(constraints):
    ...
    if len(invalid_operators) > 0:
        return (False, "Invalid constraint operator names: {}".format(operator_name))
    else:
        return (True, None)
...
class MesosTaskConfig(DefaultTaskConfigInterface):
    ...
    cmd = field(
        type=str, mandatory=True, invariant=lambda cmd: (cmd.strip() != "", "empty cmd")
    )
    cpus = field(
    ...
    gpus = field(
        type=int, initial=0, factory=int, invariant=lambda g: (g >= 0, "gpus >= 0")
    )
    timeout = field(
        type=float,
        factory=float,
    ...
    volumes = field(type=PVector, initial=v(), factory=pvector, invariant=valid_volumes)
    ports = field(
        type=(PVector[PMap] if TYPE_CHECKING else PVector), initial=v(), factory=pvector
    )
    ...
    constraints = field(
        ...
        factory=lambda c: pvector(
            (Constraint(attribute=v[0], operator=v[1], value=v[2]) for v in c)
        ),
