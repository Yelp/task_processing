...
def test_task_retry(mock_retrying_executor, mock_event):
    mock_retrying_executor.task_retries = mock_retrying_executor.task_retries.set(
        mock_event.task_id, 3
    )
...
def test_retry_loop_retries_task(mock_retrying_executor, mock_event):
    ...
    mock_retrying_executor._restore_task_id = mock.Mock(return_value=mock_event)
...
def test_retry_loop_does_not_retry_task(mock_retrying_executor, mock_event):
    ...
    modified_mock_event = mock_event.set("task_id", modified_task_id)
...
def test_retry_loop_filters_out_non_task(mock_retrying_executor):
    mock_event = Event(
        kind="control", raw="some message", message="stop", terminal=True
    )
...
def test_retry_loop_recover_attempt(mock_retrying_executor, mock_event):
    ...
    modified_mock_event = mock_event.set("task_id", original_task_id + "-retry6")
...
def test_task_config_with_retry(mock_retrying_executor, mock_task_config):
    mock_retrying_executor.task_retries = mock_retrying_executor.task_retries.set(
        mock_task_config.task_id, 2
    )

    ret_value = mock_retrying_executor._task_config_with_retry(mock_task_config)
...
def test_restore_task_id(mock_retrying_executor, mock_event):
    ...
    modified_task_config = mock_event.task_config.set(
        "uuid", str(mock_event.task_config.uuid) + "-retry1"
    )
    mock_event = mock_event.set("task_config", modified_task_config)

    ret_value = mock_retrying_executor._restore_task_id(mock_event, original_task_id)
...
def test_is_current_attempt(
    ...
    original_task_id = mock_event.task_id
    mock_retrying_executor.task_retries = mock_retrying_executor.task_retries.set(
        mock_event.task_id, 2
    )
    modified_task_id = str(mock_event.task_config.uuid) + "-retry2"
    modified_task_config = mock_event.task_config.set("uuid", modified_task_id)
    modified_mock_event = mock_event.set("task_config", modified_task_config)
    modified_mock_event = mock_event.set("task_id", modified_task_id)

    ret_value = mock_retrying_executor._is_current_attempt(
        modified_mock_event, original_task_id
    )
